...
namespace kofola {
    ...
    bool emptiness_check::gs_edited(std::shared_ptr<abstract_successor::mstate> src_mstate) {
        // stacks to replace recursion
        std::stack<std::shared_ptr<abstract_successor::mstate>> src_mstates;
        src_mstates.push(nullptr); // default
        std::stack<std::vector<std::shared_ptr<abstract_successor::mstate>>> successors;
        ...
        while(src_mstate != nullptr) {
            ...
            while(!succs.empty()) {
                ...
                if (dfs_num_[dst_mstate] == UNDEFINED && !check_simul_less(dst_mstate))
                {
                    ...
                    succs = abstr_succ_->get_succs(src_mstate);
    ...
    void emptiness_check::update_structures(std::shared_ptr<abstract_successor::mstate> src_mstate) {
    ...
    bool emptiness_check::gs(std::shared_ptr<abstract_successor::mstate> src_mstate) {
        // stacks to replace recursion
        std::stack<std::shared_ptr<abstract_successor::mstate>> src_mstates;
        src_mstates.push(nullptr); // default
        std::stack<std::vector<std::shared_ptr<abstract_successor::mstate>>> successors;
